...
export default function MyComponent({ text }) {
    return <textarea disabled style={{ resize: 'none', background: 'transparent', border: '0px none', height: '95vh', width: '100%' }} value={text}></textarea>
}
...
export async function getServerSideProps(context: GetServerSidePropsContext) {
    ...
    if (promiseBoard.status == 200) {
        ...
        if (maybeBoard.encrypted)
            text = AESDecrypt(file.value)


        return { props: { text: text } }
    }
    else
        return { props: { text: "Board not found !" } }
}
